...
ggsave(paste0("figures/mediation_analysis_BCAC_", bc_data,"_SEdiff_paper.png"),
       plot=b1, scale=1.2, 
       width=14, height=6,
       units=c("cm"), dpi=300, limitsize=F)
...
b1v3<-ggplot(mediation_results_long_out3,
  ...
  geom_errorbarh(aes(xmin=indirect_lo_ci95, xmax=indirect_up_ci95), height=.2, 
            position = ggstance::position_dodgev(height = 0.5)) +
  theme_minimal_grid(11) +
  scale_color_manual(values=c("#6A115A"))+
  #scale_color_manual(values=pal)+
  theme(strip.text = element_text(face = 'bold'),
        panel.grid.major.y = element_blank()
        #axis.text.x = element_text(angle = 25, hjust = 1)
  ...
  labs(color = "",
       ...
       y = "", x = "Effect size (beta)"
       #subtitle="                indirect effect of childhood body size via mediator ",
       #title= "            Mediation analysis (Product method + Delta SE method)" 
       )+
  theme(legend.position = "none",  plot.title.position = "plot")
ggsave(paste0("figures/mediation_analysis_BCAC_", bc_data,"_paper_main.png"),
       plot=b1v3, scale=1.2, 
       width=11, height=6,
       units=c("cm"), dpi=300, limitsize=F)
