...
if (source == "textfiles"){
  ...
  for (i in 1:length(mediators)){
    ...
    if (is.null(instruments)){ instruments <- extract_instruments(mediators[i], p1 = 10e-07)} # if no SNPs returned, try lower pval 
...
dat <-filter(dat, trait_type != "non-dense") # excluding due to low number of SNPs
p<-ggplot(dat, aes(y=exposure, x=or, label=outcome, colour=trait_type, shape=method)) +
  geom_errorbarh(aes(xmin=or_lci95, xmax=or_uci95), height=.3) +
  geom_point(size=2)+
  scale_color_manual(values=pal)+
...
merged_data <- merged_data %>% 
              filter(exposure %in% 
                      ...
                      "fasting glucose (Lagou)", "fasting insulin (Lagou)",  "HBa1c (Wheeler)", "HOMA-B", "HOMA-IR",
                       "Breast size (Ooi 2019)", "Percent mammographic density  (Brand 2018)", "Mammographic density (dense area) (Brand 2018)")) %>% 
              ...
              mutate(exposure = case_when(exposure == "fasting insulin" ~ "Fasting Insulin",
                                         ...
                                         exposure == "IGF" ~ "IGF-1",
                                         exposure == "Percent mammographic density " ~ "Mammographic density (%)",
...
p2<-ggplot(merged_data, aes(y=exposure, x=or, label=outcome , group = trait_category, colour=trait_category)) +
  geom_errorbarh(aes(xmin=or_lci95, xmax=or_uci95), height=.3) +
