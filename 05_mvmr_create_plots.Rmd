...
## subset case; special rules
#merged_data$exposure <- forcats::fct_rev(forcats::fct_inorder(merged_data$exposure))
#pal<-c(wes_palette("Darjeeling1")[c(4,2,3)]) # 3 1 4 2
#pX<-ggplot(merged_data, aes(y=exposure, x=or, label=outcome , group = trait_category, colour=trait_category)) +
#  geom_errorbarh(aes(xmin=or_lci95, xmax=or_uci95), height=.2) +
#  geom_point(size=2, shape=20)+
#  scale_color_manual(values=pal)+
#  #xlim(0.43, 2.5)+
...
# create a new label
dat$direct <- paste0("Childhood BMI accounted for " , dat$pair)
...
p2<-ggplot(dat_BMI, aes(y=reorder(direct, -or), x=or, label=outcome, colour=bmi_no_bmi)) +
  geom_errorbarh(aes(xmin=or_lci95, xmax=or_uci95), height=.3) +
  geom_point(size=2)+
  #geom_vline(xintercept=cBMI_OR, linetype=2, colour = "#FF0000") +
  ...
  #facet_grid(trait_category~outcome, scale="free", switch="x")+
  labs(color = "",y = "", x = "Odds ratio", subtitle="",
       title="          MVMR: direct effect of childhood BMI on breast cancer (OR, 95% CI) ")+
  theme(legend.position = "none")+#, plot.title.position = "plot")+
  theme(axis.text.y = element_text(colour = dat_BMI$text_col))

ggsave(paste0("figures/merged_cBMI_direct-to-BCAC_", bc_data,"_IVW", mvmr_method,".png"),
       plot=p2, scale=1.2, 
       width=12, height=10,
       units=c("cm"), dpi=300, limitsize=F)
...
## new analysis: try putting together estimates from all different parts
...
data_all_reshaped<-
  data_all %>% 
    ...
    mutate(analysis_type= case_when(analysis_type == "data1" ~ "MVMR: childhood BMI + mediator",
                                    analysis_type == "data2" ~ "MVMR: childhood BMI + adult BMI + mediator",
                                    analysis_type == "data3" ~ "MVMR: childhood BMI + childhood height + mediator", 
                                    TRUE ~ analysis_type))
  
# set the order in which they apper in th eplot
data_all_reshaped$analysis_type <- factor(data_all_reshaped$analysis_type ,
                               levels = rev(c("MVMR: childhood BMI + mediator", 
                                              "MVMR: childhood BMI + adult BMI + mediator",
                                              "MVMR: childhood BMI + childhood height + mediator")))
...
dat_mv <- read_tsv(paste0(results_path, current_trait_category, "/merged/mvmr_BMIs-BCAC_", bc_data,".tsv")) %>% 
